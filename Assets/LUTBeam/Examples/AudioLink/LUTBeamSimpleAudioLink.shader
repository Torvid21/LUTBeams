Shader "LUTBeam/SimpleAudioLink"
{
    Properties
    {
        ...
        [KeywordEnum(Low,MidLow,MidHigh,High)] _Band ("Band", Float) = 0
        [IntRange] _Delay ("Delay", Range(0, 127)) = 0

        [Toggle] _EnableThemeColorSampling ("Enable Theme Color Sampling", Float) = 0
        [KeywordEnum(Theme0,Theme1,Theme2,Theme3)] _ThemeColorTarget ("Theme Color Target", Int) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float _Band;
            float _Delay;
            float _EnableThemeColorSampling;
            float _ThemeColorTarget;

            float GetAudioAmplitube()
            {
                return AudioLinkLerp(ALPASS_AUDIOLINK + float2(_Delay, _Band)).rrrr;
            }

            float4 GetThemeColor()
            {
                switch((int)_ThemeColorTarget)
                {
                    case 0: return AudioLinkData(ALPASS_THEME_COLOR0);
                    case 1: return AudioLinkData(ALPASS_THEME_COLOR1);
                    case 2: return AudioLinkData(ALPASS_THEME_COLOR2);
                    case 3: return AudioLinkData(ALPASS_THEME_COLOR3);
            ...
            {
                float4 col = _EmissionColor;
                if (_EnableAudioLink > 0.5)
                {
                    col *= GetAudioAmplitube();
                    if (_EnableThemeColorSampling > 0.5)
                    {
                        col *= GetThemeColor();
                    }
                }
            ...
            v2f vert(appdata v)
            {
                ...
                settings.color = GetEmissionColor();
